...
export const logingIn = async (req, res) => {
  ...
  try {
    ...
    // send email
    const mailObj = {
      mailFrom: `blogPlatform ${process.env.EMAIL_USER}`,
      mailTo: email,
      subject: "successfully logged in",
      body: `<h1> Welcome to blogPlatform ${userName} </h1>
    ...
    return res
      ...
      .status(200)
      .json({ message: "Login Successful proceed to post a blog" });
  } catch (error) {
...
export const passwordResetRequest = async (req, res) => {
  ...
  try {
    ...
    await sendMail({
      ...
      body: `<p>Click on this link to reset your password </p>  <a href="https://localhost:1000/password/reset/${token}">Reset Password</a>`,
    });
    res
      .status(200)
